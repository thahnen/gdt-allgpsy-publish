...
if ($parameter[0] == "standard") {
    ...
    else { try { $parameter[2] = intval($lanzahl_ergebnisse); }
        catch (Exception $e) { goto fehler; }
    }
    ...
    else { try { $parameter[5] = intval($lguthaben); }
        catch (Exception $e) { goto fehler; }
    }
    ...
    for ($i = 0; $i < $parameter[2]; $i++) { $txt = $txt . ($i+1) . "\t\t" . $parameter[4][$i] . "\t\t" . $parameter[3][$i] . "\n"; }
...
?>
    ...
    <script>window.onload = function() { history.replaceState({}, document.title, "/ende.php"); };</script>
</body>
</html>